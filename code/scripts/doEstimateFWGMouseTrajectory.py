...
def main(argv):
    ...
    parser.add_argument("simRes_number", help="simulation result number", type=int)
    parser.add_argument("estMeta_number", help="estimation metadata number", type=int)
    ...
    parser.add_argument("--estRes_data_filename_pattern", type=str,
                        default="../../results/{:08d}_estimation.npz",
    ...
    simRes = np.load(simRes_filename_pattern.format(simRes_number))
    ...
    V0_diag_value = float(estMeta["initial_params"]["v0_diag_value"])
    em_max_iter = int(estMeta["optim_params"]["em_max_iter"])

    R_0 = np.diag([sigma_x0**2, sigma_y0**2])
    # diag_R_0 = np.array([sigma_x0**2, sigma_y0**2])
    ...
    # diag_V0_0 = np.array([V0_diag_value for i in range(len(m0_0))])
    ...
    # R, m0, diag_V0, sigma_a = learning.optim_SS_tracking_DWPA_diagV0(y=simRes["y"], B=simRes["B"], sigma_a0=sigma_a0, Qt=simRes["Qt"], Z=simRes["Z"], diag_R_0=diag_R_0, m0_0=m0_0, diag_V0_0=diag_V0_0, max_iter=em_max_iter)
...
#     estRes_data_filename = estRes_data_filename_pattern.format(estRes_number)
    ...
    np.savez(estRes_data_filename, R=R, m0=m0, V0=V0, sigma_a=sigma_a)
    print("Saved results to {:s}".format(estRes_metadata_filename))
