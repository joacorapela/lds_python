...
def main(argv):
    ...
    parser.add_argument("--noise_intensity_old", type=float, default=1.3,
                        help="old value for noise_intensity")
    ...
    parser.add_argument("--fig_filename_pattern", type=str,
                        default=("../../../figures/{:08d}_lowerBound_noise_intensity_"
                                 "sweep.{:s}"),
                        help="figure filename pattern")
    args = parser.parse_args()

    simRes_number = args.simRes_number
    noise_intensity_old = args.noise_intensity_old
    ...
    noise_intensitys = np.arange(noise_intensity_min, noise_intensity_max, noise_intensity_step)
    lower_bounds = np.empty(len(noise_intensitys))
    for i, noise_intensity in enumerate(noise_intensitys):
        lower_bounds[i] = -N*M/2*np.log(noise_intensity)-K/(2*noise_intensity)
        print(f"lower bound for noise_intensity={noise_intensity:.02f}: {lower_bounds[i]}")
    fig = go.Figure()
    trace = go.Scatter(x=noise_intensitys, y=lower_bounds, mode="lines+markers")
    fig.add_trace(trace)
